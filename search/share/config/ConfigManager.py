...
class ConfigManager(object):
    ...
    def load_config(self, key="all"):
        ...
        try:
            if key == "all":
                debug_logger.info('Read [%s] config from %s', key, self.default_conf_file)
    ...
    def update_db_config(self, db_config, remote_config):
        ...
        try:
            ...
            debug_logger.critical('Failed to to fetch mongo_uri from remote host, using local mongo_uri')
    ...
    def read_key_from_file(file_path):
        ...
        try:
            ...
            debug_logger.critical('Failed to read api key from file %s', file_path)
            return None
