(function ($) {
  ...
  KorePickers.prototype.showradioOptionsPicker = function (mainConfig) {
    ...
    _korePickers
      .find('.kore-action-sheet')
      .append(this.getradioOptionsPickerContainer());
    this.addAccountIconToFooter();
    _korePickers
      .find('.radioOptionsPickerContainer')
      .append(this.getradioOptionsPickerTemplate(accountData));
    this.addradioOptionsMenuListener(mainConfig);
  };
  KorePickers.prototype.getradioOptionsPickerTemplate = function (
    radioOptionConfig
  ) {
    ...
    radioOptions.forEach(function (radioOption) {
      var radioOptionHtml = $(
        '<label class="radioButton"><div class="btnAccount">\
      <div class="radioValue">\
         <input type="radio" id="selectedValue" name="radio">\
         <span class="checkmark"></span>\
         </div>\
         <span class="radioOptionDetails">\
            <span class="radioOptionName" title="' +
          radioOption.title +
          '" data-value="' +
          radioOption.postback.value +
          '" data-title="' +
          radioOption.postback.title +
          '">' +
          radioOption.title +
          '</span>\
            <div class="radioOptionValue" title="' +
          radioOption.value +
          '">' +
          radioOption.value +
          '</div>\
  ...
  KorePickers.prototype.addradioOptionsMenuListener = function (mainConfig) {
    ...
    _korePickers
      .find('.kore-action-sheet .dateRangePickerContainer')
      .addClass('hide');
    ...
    _korePickers
      .find('.kore-action-sheet  .datePickerContainer')
      .addClass('hide');
    ...
    _self.bottomSlider(
      'show',
      _korePickers.find('.radioOptionsPickerContainer')
    );
    setTimeout(function () {
      _korePickers.find('.radioOptionsPickerContainer .confirmBTN').css({
        display: 'block',
    ...
    _korePickers
      .find('.radioOptionsPickerContainer .confirmBTN')
      .click(function (event) {
        var radioOptionPostbackMsg = target
          .find('.radioOptionName')
          .attr('data-value');
        var radioOptionTitle = target
          .find('.radioOptionName')
          .attr('data-title');
        mainConfig[0].chatWindowInstance.sendMessage(
          $('.chatInputBox').text(radioOptionPostbackMsg),
          radioOptionTitle
        );
        _korePickers
          .find('.radioOptionsPickerContainer .confirmBTN')
    ...
    _korePickers
      .find('.radioOptionsPickerContainer .closePicker')
      .click(function () {
        _korePickers.find('.radioOptionsPickerContainer .confirmBTN').css({
          display: 'none',
        });
  ...
  KorePickers.prototype.getAccountIconTemplate = function () {
    var AccountFooterTemplate =
      '<div class="sdkFooterIcon account"> \
  ...
  KorePickers.prototype.addAccountIconToFooter = function () {
    _korePickers
      .find('.radioOptionsPickerContainer')
      .append(this.getAccountIconTemplate);
  };
  ...
  KorePickers.prototype.showTaskPicker = function (mainConfig) {
    ...
    _korePickers
      .find('.kore-action-sheet')
      .append(this.getTaskPickerContainer());
    _korePickers
      .find('.TaskPickerContainer')
      .append(this.getTaskPickerTemplate(actionsData));
  ...
  KorePickers.prototype.getTaskPickerTemplate = function (taskPickerConfig) {
    ...
    tasks.forEach(function (task) {
      var taskHtml = $(
        '<div class="btnTask">\
        <span class="taskName" data-value="' +
          task.postback.value +
          '" data-title="' +
          task.postback.title +
          '" title="' +
          ...
          task.title +
          '</span>\
        <div class="imageIcon"> <img src="' +
          task.icon +
          '" class="displayIcon"></div>\
        </div>'
      );
  ...
  KorePickers.prototype.addTaskMenuListener = function (mainConfig) {
    ...
    _korePickers.find('.TaskPickerContainer .btnTask').click(function (e) {
      ...
      _korePickers
        .find('.footerContainer.pos-relative .chatInputBox')
        .text(inputValue);
      var taskPostbackMsg = $(e.currentTarget)
        .find('.taskName')
        .attr('data-value');
      var taskTitle = $(e.currentTarget).find('.taskName').attr('data-title');
      mainConfig[0].chatWindowInstance.sendMessage(
        $('.chatInputBox').text(taskPostbackMsg),
        taskTitle
      );
      _self.bottomSlider('hide');
      _korePickers.find('.kore-action-sheet').remove();
  ...
  KorePickers.prototype.getTaskIconTemplate = function () {
    var TaskFooterTemplate =
      '<div class="sdkFooterIcon menu"> \
  ...
  KorePickers.prototype.addTaskIconToFooter = function () {
    let _korePickers = this._korePickers;
  ...
  KorePickers.prototype.showDateRangePicker = function (mainConfig) {
    ...
    _korePickers
      .find('.kore-action-sheet')
      .append(this.getdateRangePickerContainer);
    ...
    var defaultlibConfig = {
      ...
      inline: true,
      container: _korePickers.find(
        '.kore-action-sheet .dateRangePickerContainer'
      ),
    };
    $.extend(defaultlibConfig, mainConfig[1].dateRangeConfig);
    daterangeInput = _korePickers
      .find('.kore-action-sheet .dateRangePickerContainer #rangeCalenderBtn')
      .dateRangePicker(defaultlibConfig);
  ...
  KorePickers.prototype.addDateRangeListener = function (mainConfig) {
    ...
    daterangeInput.bind(
      ...
      function (event, obj) {
        ...
        showStartYearValue = _korePickers
          .find(
            '.kore-action-sheet .dateRangePickerContainer .showStartdate .showStartYear'
          )
          .html(startYear);
        showStartDateValue = _korePickers
          .find(
            '.kore-action-sheet .dateRangePickerContainer .showStartdate .showStartMonth'
          )
          .html(startDate);
        _korePickers
          .find(
            '.kore-action-sheet .dateRangePickerContainer .showEnddate .showEndMonth'
          )
          .html('Select');
        _korePickers
          .find(
            '.kore-action-sheet .dateRangePickerContainer .showEnddate .showEndYear'
          )
          .css({ opacity: '0' });
      }
    );
    daterangeInput.bind('datepicker-change', function (event, obj) {
      ...
      showEndYearValue = _korePickers
        .find(
          '.kore-action-sheet .dateRangePickerContainer .showEnddate .showEndYear'
        )
        .html(endYear);
      var endDate = moment(enddateValue).format('ddd,MMM DD');
      showEndDateValue = _korePickers
        .find(
          '.kore-action-sheet .dateRangePickerContainer .showEnddate .showEndMonth'
        )
        .html(endDate);
    ...
    _korePickers
      .find('.kore-action-sheet .dateRangePickerContainer .confirmBTN')
      .click(function () {
        if (showStartDateValue !== null && showEndDateValue == null) {
          ...
          var startViewDate =
            moment(startdateValue).format('MM-DD-YYYY') || moment();
          var endViewDate =
            moment(enddateValue).format('MM-DD-YYYY') || moment();
    ...
    _korePickers
      .find('.kore-action-sheet .dateRangePickerContainer .close-button')
      .click(function () {
        _self.bottomSlider('hide');
        _korePickers.find('.kore-action-sheet').remove();
      });
  ...
  KorePickers.prototype.addClickEventRangeCalender = function (mainConfig) {
    ...
    _korePickers
      ...
      .addClass('hide');
    _korePickers
      .find('.kore-action-sheet .clockPickerContainer')
      .addClass('hide');
    _korePickers
      .find('.kore-action-sheet .radioOptionsPickerContainer')
      .addClass('hide');
    _korePickers
      .find('.kore-action-sheet .TaskPickerContainer')
      .addClass('hide');
    _korePickers.find('.listViewTmplContent').hide();
    _korePickers.find('.list-template-sheet').hide();
    _korePickers.find('.listTmplContent.advancedMultiSelect').hide();
    ...
    _korePickers
      ...
      .removeClass('hide');
    _korePickers
      .find('.kore-action-sheet .dateRangePickerContainer .date-picker-wrapper')
      .css({ border: '0px', 'background-color': 'white' });
    _korePickers
      .find('.kore-action-sheet .dateRangePickerContainer .month-wrapper')
      .css({ border: '0px' });
    _korePickers
      .find(
        '.kore-action-sheet .dateRangePickerContainer .headerCalendar .choose'
      )
      .html(mainConfig[1].daterangepicker.title);
  ...
  KorePickers.prototype.getDateRangeIconTemplate = function () {
    var dateRangeFooterTemplate =
      '<div class="sdkFooterIcon rangeCalender"> \
  ...
  KorePickers.prototype.addDateRangeIconToFooter = function () {
    _korePickers
      .find('.kore-action-sheet .dateRangePickerContainer')
      .append(this.addDateRangeIconTemplate);
  };
  ...
  KorePickers.prototype.showDatePicker = function (mainConfig) {
    ...
    daterangeInput = _korePickers
      .find('.kore-action-sheet .datePickerContainer #calender')
      .dateRangePicker(defaultlibConfig);
  ...
  KorePickers.prototype.addDateListener = function (mainConfig) {
    ...
    daterangeInput.bind(
      ...
      function (event, obj) {
        ...
        showStartYearValue = _korePickers
          .find(
            '.kore-action-sheet .datePickerContainer .showStartdate .showStartYear'
          )
          .html(startYear);
        showStartDateValue = _korePickers
          .find(
            '.kore-action-sheet .datePickerContainer .showStartdate .showStartMonth'
          )
          .html(startDate);
      }
    );
    _korePickers
      .find('.kore-action-sheet .datePickerContainer .confirmBTN')
      .click(function () {
        var startViewDate =
          moment(startdateValue).format('MM-DD-YYYY') || moment();
    ...
    _korePickers
      .find('.kore-action-sheet .datePickerContainer .close-button')
      .click(function () {
        _self.bottomSlider('hide');
        _korePickers.find('.kore-action-sheet').remove();
      });
  ...
  KorePickers.prototype.addClickEventCalender = function (mainConfig) {
    ...
    _korePickers
      ...
      .removeClass('hide');
    _korePickers
      .find('.kore-action-sheet  .datePickerContainer .date-picker-wrapper')
      .css({ border: '0px', 'background-color': 'white' });
    _korePickers
      .find('.kore-action-sheet  .datePickerContainer .month-wrapper')
      .css({ border: '0px' });
    _korePickers
      .find('.kore-action-sheet .datePickerContainer  .headerCalendar .choose')
      .html(mainConfig[1].datepicker.title);
    _korePickers
      .find('.kore-action-sheet .datePickerContainer  .confirmBTN')
      .css({
        display: 'none',
  ...
  KorePickers.prototype.getDateIconTemplate = function () {
    var dateFooterTemplate =
      '<div class="sdkFooterIcon singleCalender"> \
  ...
  KorePickers.prototype.addDateIconToFooter = function () {
    _korePickers
      .find('.kore-action-sheet  .datePickerContainer')
      .append(this.addDateIconTemplate);
  };
  ...
  KorePickers.prototype.showClockPicker = function (mainConfig, e) {
    ...
    _korePickers
      .find('.kore-action-sheet')
      .append(this.getclockPickerContainer);
  ...
  KorePickers.prototype.addClickEventClock = function (mainConfig) {
    ...
    if (mainConfig[1].clockPicker.title) {
      _korePickers
        .find('.kore-action-sheet .clockPickerContainer .headerClock .choose')
        .html(mainConfig[1].clockPicker.title);
    } else {
      _korePickers
        .find('.kore-action-sheet .clockPickerContainer .headerClock .choose')
        .html('Please Choose');
    ...
    _korePickers
      .find('.btn.btn-sm.btn-default.btn-block.clockpicker-button')
      .hide();
    setTimeout(function () {
      _korePickers
        .find('.kore-action-sheet .clockPickerContainer .clockpicker-popover')
        .css({ display: 'block', top: '75px ', left: '80.5px' });
      _korePickers
        .find(
          '.kore-action-sheet .clockPickerContainer .clockpicker-popover .clockpicker-plate'
        )
        .css({ 'margin-left': '25px' });
      _korePickers
        .find('.btn.btn-sm.btn-default.clockpicker-button.pm-button')
        .css({ opacity: '0.4' });
    }, 0);
  };
  KorePickers.prototype.addClockListener = function (mainConfig) {
    ...
    _korePickers
      .find('.btn.btn-sm.btn-default.clockpicker-button.am-button')
      .click(function () {
        _korePickers
          .find('.btn.btn-sm.btn-default.clockpicker-button.pm-button')
          .css({ opacity: '0.4' });
    ...
    _korePickers.find('.clockPickerContainer .confirmBTN').click(function () {
      _korePickers
        .find('.clockPickerContainer .confirmBTN')
        .css({ display: 'block' });
      _korePickers
        .find('.btn.btn-sm.btn-default.btn-block.clockpicker-button')
        .trigger('click');
  ...
  KorePickers.prototype.getClockIconTemplate = function () {
    var clockFooterTemplate =
      '<div class="sdkFooterIcon clock"> \
  ...
  KorePickers.prototype.addClockIconToFooter = function () {
    _korePickers
      .find('.clockPickerContainer')
      .append(this.getClockIconTemplate);
  };
