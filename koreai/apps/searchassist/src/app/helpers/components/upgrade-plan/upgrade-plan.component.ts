...
export class UpgradePlanComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.currentSubsciptionData =
      this.appSelectionService.currentSubscription.subscribe((res) => {
        ...
        const billingUnit = this.selectedPlan?.billingUnit
  ...
  getAllPlans() {
    this.service.invoke('get.pricingPlans').subscribe(
      (res) => {
        ...
        this.totalPlansData = res?.plans?.sort((a, b) => {
    ...
    this.service.invoke('get.payementStatus', queryParams).subscribe(
      (res) => {
        if (res.state == 'success') {
          ...
          const message = this.isOverageShow
            ? 'Overages added successfully'
  ...
  typeOfPlan(type?) {
    const billingUnit = type
      ? type
      : this.selectedPlan?.billingUnit || 'Monthly';
    ...
    if (this.totalPlansData.length > 0) {
      ...
      listData.forEach((data) => {
        ...
        if (
          data.planId == this.plansIdList.free ||
          data.type == this.plansIdList.standardMonth ||
          data.type == this.plansIdList.enterpriceMonth
        ) {
          ...
          for (let j = 0; j < listDataMonthlyFeature[i].length; j++) {
            if (listDataMonthlyFeature[i][j]) {
              if (
                ...
                //comapre 3 records with 1st record's Key
                listDataMonthlyFeature[0][j].push(
                  listDataMonthlyFeature[i][j][1]
                ); // push the values array in 1st record
              }
            }
          }
        }
  ...
  buyOverage() {
    ...
    if (this.overageDeatils.docCount > 0) {
      overage.push({
        feature: 'ingestDocs',
        quantity: this.overageDeatils.docCount,
      });
    }
    if (this.overageDeatils.queriesCount > 0) {
      overage.push({
        feature: 'searchQueries',
        quantity: this.overageDeatils.queriesCount,
      });
    }
    const queryParams = {
      streamId: this.selectedApp._id,
