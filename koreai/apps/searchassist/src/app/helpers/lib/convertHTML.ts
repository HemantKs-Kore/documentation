  ...
  public helpers = {
    ...
    checkMarkdowns(val: any, hyperLinksMap: { [x: string]: any }) {
      ...
      for (let i = 0; i < txtArr.length; i++) {
        ...
        const _matchImage = txtArr[i].match(
          /\!\[([^\]]+)\](|\s)+\(([^\)])+\)/g
        );
        if (_matchImage && _matchImage.length > 0) {
          for (j = 0; j < _matchImage.length; j++) {
            const _imgTxt = _matchImage[j].substring(
              2,
              _matchImage[j].indexOf(']')
            );
            const remainingString = _matchImage[j]
              .substring(_matchImage[j].indexOf(']') + 1)
              .trim();
            let _imgLink = remainingString.substring(
              1,
        ...
        const _matchLink = txtArr[i].match(
          /\[([^\]]+)\](|\s)\((?:[^)(]+|\((?:[^)(]+|\([^)(]*\))*\))*\)?/g
        );
        if (_matchLink && _matchLink.length > 0) {
          for (j = 0; j < _matchLink.length; j++) {
            const _linkTxt = _matchLink[j].substring(
              1,
              _matchLink[j].indexOf(']')
            );
            const remainingString = _matchLink[j]
              .substring(_matchLink[j].indexOf(']') + 1)
              .trim();
            let _linkLink = remainingString.substring(
              1,
              remainingString.indexOf(')')
            );
            ...
            _linkLink =
              '<span class="isLink"><a href="' +
              _linkLink +
              '" target="underscoreblank">' +
              this.checkMarkdowns(_linkTxt, null) +
              '</a></span>';
