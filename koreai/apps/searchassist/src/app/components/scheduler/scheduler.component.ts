...
export class SchedulerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.crwalObject) {
      ...
      if (this.crwalObject?.interval?.intervalValue) {
        this.endsFreq(
          this.crwalObject?.interval?.intervalValue?.endsOn?.endType,
          'set'
        );
        this.repeatEvery = this.crwalObject?.interval?.intervalValue?.every
          ? this.crwalObject?.interval?.intervalValue?.every
          : this.repeatEvery;
        this.custFreq = this.crwalObject?.interval?.intervalValue
          ?.schedulePeriod
          ? this.crwalObject?.interval?.intervalValue?.schedulePeriod
          : this.custFreq;
        this.weeKDay = this.crwalObject?.interval?.intervalValue?.repeatOn
          ? this.crwalObject?.interval?.intervalValue?.repeatOn
          : this.weeKDay;
        this.endDate = this.crwalObject?.interval?.intervalValue?.endsOn
          ?.endDate
          ? this.crwalObject?.interval?.intervalValue?.endsOn?.endDate
          : this.endDate;
        this.occurence = this.crwalObject?.interval?.intervalValue?.endsOn
          ?.occurrences
          ? this.crwalObject?.interval?.intervalValue?.endsOn?.occurrences
          : this.occurence;
      }
    }
  }
  ...
  calculateCronExpression() {
    ...
    if (this.rstz == 'Does not repeat') {
      ...
      this.cronExpression =
        this.timeMM + ' ' + timeHH + ' *' + ' * ' + ' MON,TUE,WED,THU,FRI';
  ...
  customFrequency(timeHH) {
    ...
    if (
      ...
      this.cronExpression =
        this.timeMM + ' ' + timeHH + ' */' + this.repeatEvery + ' * ' + '*';
  ...
  scheduleEmittFunc() {
    ...
    intervalValue.schedulePeriod =
      this.lower(this.custFreq).charAt(this.lower(this.custFreq).length - 1) ==
      's'
        ? this.lower(this.custFreq)
        : this.lower(this.custFreq + 's');
    intervalValue.repeatOn =
      this.custFreq == 'Weeks' ||
      this.custFreq == 'Week' ||
      this.custFreq == 'weeks'
        ? this.weeKDay
    ...
    endsOn.occurrences =
      Number(this.occurence) > 0 ? Number(this.occurence) : null;
  ...
  openCloseSchedular(type) {
    if (type === 'open') {
      ...
      this.isCustom = false;
      if (this.schedualarDataModelPopRef?.close)
        this.schedualarDataModelPopRef.close();
    }
  }
