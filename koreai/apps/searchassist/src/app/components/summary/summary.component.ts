  ...
  getAllOverview(status?) {
    ...
    this.service.invoke('get.overview', queryParams).subscribe(
      res => {
        ...
        this.show_indices = (this.indices.botActions.tasks > 0 || this.indices?.connectors?.sources > 0 || this.indices.files > 0 || this.indices.structuredDataCount > 0 || this.indices.web.domains > 0 || this.indices.web.numOfDocs > 0 || this.indices.faqs.in_review > 0 || this.indices.faqs.draft > 0 || this.indices.faqs.approved > 0) ? true : false;
        if (status == undefined) {
          let subscription_data = this.appSelectionService?.currentsubscriptionPlanDetails;
          this.currentPlan = subscription_data?.subscription;
        }
