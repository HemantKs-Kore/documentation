...
export class AddFaqComponent implements OnInit, OnDestroy {
    ...
    this.altAddSub = this.faqServiceAlt.addAltQues.subscribe((params: any) => {
      ...
      if (!(this.faqData || {})._id) {
        if (!this.faqData) {
          ...
          this.faqData._source.faq_alt_questions.push(
            params._source.faq_alt_questions[0]
          );
        }
        this.isAdd = true;
      }
  ...
  checkDuplicateConditions(suggestion: string = '', conditions): boolean {
    return conditions.every(
      (f) => f.value.toLowerCase() !== suggestion.toLowerCase()
    );
  }
  addConditionTag(event: MatChipInputEvent, faqObj) {
    ...
    if ((value || '').trim()) {
      if (
        !this.checkDuplicateConditions((value || '').trim(), faqObj.conditions)
      ) {
        this.notify.notify('Duplicate tags are not allowed', 'warning');
      } else {
        const conditionTags: any = JSON.parse(
          JSON.stringify(this.conditionInterface)
        );
  ...
  changeResponseType(faqObj, index) {
    faqObj.answerType =
      faqObj.answerType === 'condition' ? 'default' : 'condition';
    if (
      faqObj.answerType === 'condition' &&
      !(faqObj.conditions && faqObj.conditions.length)
    ) {
  ...
  addAnotherResponse(type) {
    if (type === 'default') {
      const tempResponseObj = JSON.parse(
        JSON.stringify(this.defaultAnsInterface)
      );
      ...
      const tempResponseObj = JSON.parse(
        JSON.stringify(this.conditionalAnsInterface)
      );
  ...
  initializeEditResponselayoutEvents() {
    ...
    $('.add-faq-modal-popup')
      ...
      .on('click', function (event) {
        if (
          !$(event.target).closest('.text-area-editor').length &&
          !$(event.target).closest('.provideLinkPopup').length &&
          $('.editResponseMode').hasClass('focusedEdit')
        ) {
  ...
  delete(index) {
    if (
      this.faqResponse &&
      this.faqResponse.defaultAnswers &&
      this.faqResponse.defaultAnswers.length > 1
    ) {
      this.faqResponse.defaultAnswers.splice(index, 1);
    } else if (
      this.faqResponse &&
      this.faqResponse.conditionalAnswers &&
      this.faqResponse.conditionalAnswers.length >= 1
    ) {
  ...
  setDataforEditDelete(faqdata) {
    if (faqdata) {
      if (
        faqdata &&
        faqdata._source &&
        faqdata._source.faq_answer &&
        faqdata._source.faq_answer.length
      ) {
      ...
      if (
        faqdata &&
        faqdata._source &&
        faqdata._source.faq_cond_answers &&
        faqdata._source.faq_cond_answers &&
        faqdata._source.faq_cond_answers.length
      ) {
  ...
  prpaerFaqsResponsePayload() {
    ...
    if (
      this.faqResponse &&
      this.faqResponse.defaultAnswers &&
      this.faqResponse.defaultAnswers.length
    ) {
    ...
    if (
      this.faqResponse &&
      this.faqResponse.conditionalAnswers &&
      this.faqResponse.conditionalAnswers.length
    ) {
  ...
  save() {
    ...
    if (
      this.anwerPayloadObj.defaultAnswers &&
      this.anwerPayloadObj.defaultAnswers.length
    ) {
      const oneValidRespone = _.filter(
        this.anwerPayloadObj.defaultAnswers,
        (answer) => {
          delete answer.type;
          return (
    ...
    const emmiter =
      this.faqData && !this.addAltFaq ? this.editFaq : this.addFaq;
    this.loading = true;
    const payload = {
      _source: {
        ...
        alternateQuestions:
          this.faqData &&
          this.faqData._source &&
          this.faqData._source.faq_alt_questions
            ? this.faqData._source.faq_alt_questions
            : this.addAltFaq &&
              this.addAltFaq._source &&
              this.addAltFaq._source.faq_alt_questions
            ? this.addAltFaq._source.faq_alt_questions
            : [],
      },
      followupQuestions:
        this.faqData && this.faqData.followupQuestions
          ? this.faqData.followupQuestions
          : [],
  ...
  addImage() {
    ...
    this.faqResponse.defaultAnswers.forEach((element, index) => {
      if (this.faqResponse.defaultAnswers.length - 1 == index) {
        (element || {})['image_url'] =
          this.imgInfo && this.imgInfo.url ? this.imgInfo.url : '';
      }
    });
  ...
  replaceAt(range, replacement, mainText) {
    ...
    return (
      mainText.substring(0, range.startIndex) +
      replacement +
      mainText.substring(range.endIndex)
    );
  }
  handleToolBarAction(text, range) {
    ...
    if (this.contentEditableElement) {
      ...
      if (
        this.selectedResponseToEdit &&
        this.selectedResponseToEdit.index > -1
      ) {
  ...
  addFollowupQues() {
    this.isAlt = true;
    this.followInpKeySub = this.faqServiceFollow.inpKeywordsAdd.subscribe(
      (res) => {
        if (this.quesList.followupQuestions[0]) {
          this.quesList.followupQuestions[0].keywords = _.map(res, (o) => {
            return { keyword: o };
          });
        } else {
    ...
    this.followInpQuesSub = this.faqServiceFollow.inpQuesAdd.subscribe(
      (res) => {
        if (this.quesList.followupQuestions[0]) {
