...
export class AppUrlsService {
  ...
  public redirectToLogin() {
    ...
    setTimeout(() => {
      ...
      if(localStorage.getItem('utmLocation')){
        window.location.href = redirectUrl + localStorage.getItem('utmLocation');
      }else{
  ...
  public campaignParams() {
    ...
    if (JSON.stringify(params) !== "{}" || JSON.stringify(first_params) !== "{}") {
      window.localStorage.setItem('utmDetails', JSON.stringify(params));
      window.localStorage.setItem('utmDetails_first', JSON.stringify(first_params));
    }
  }
