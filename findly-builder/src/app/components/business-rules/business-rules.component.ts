...
export class BusinessRulesComponent implements OnInit, OnDestroy {
  ...
  createRule() {
    ...
    if (this.selcectionObj?.ruleType === 'nlp') {
      isValidate = this.validateRules() && this.validateOut() && this.emptyAnnotatorValidate() && this.validateLegends('all');
    }
  ...
  validateLegends(type) {
    let entityArray=[], isValid=false, count=0;
      ...
      if(isCountEqual){
        this.nlpAnnotatorObj.annotator.forEach(annotator=>{
          annotator.entities.forEach(item1=>{
            const entityObj = this.sys_entities.filter(item => item._id === item1.entityId);
          if(!entityArray.includes(entityObj[0].entityName)) entityArray.push(entityObj[0].entityName);
          })
        })
      }else{
        isValid = false;
    ...
    if(!isValid) this.errorToaster(null, 'Tags should equally matched with legends in each annotator');
    return isValid;
  }
  ...
  selectEntity(entity) {
    ...
    if (this.selectEditIndex !== null) {
      this.nlpAnnotatorObj.annotator[this.selectEditIndex].colorSentence = this.updateColorSentence();
    } else {
  ...
  saveEntity() {
    let isPush = false;
    let entityArray = (this.selectEditIndex !== null) ? this.nlpAnnotatorObj.annotator[this.selectEditIndex].entities : this.entityObj.entities;
    if (entityArray.length > 0) {
  ...
  updateColorSentence() {
    ...
    if (this.selectEditIndex !== null) {
      this.nlpAnnotatorObj.annotator[this.selectEditIndex].entities = total_entities;
    } else {
  ...
  AddSelectedEnity() {
    if (this.validateEntityObject()) {
      ...
      if (this.selectEditIndex !== null) {
        ...
        isCall = this.nlpAnnotatorObj.annotator.length===0?true:this.validateLegends('single');
        if(isCall){
  ...
  validateEntityObject() {
    ...
    const text: string = (this.selectEditIndex !== null) ? (this.nlpAnnotatorObj.annotator[this.selectEditIndex].sentence) : (this.entityObj.sentence);
    const entities = (this.selectEditIndex !== null) ? (this.nlpAnnotatorObj.annotator[this.selectEditIndex].entities) : (this.entityObj.entities);
    if (text.length === 0) {
  ...
  chooseEntity(type, entity?) {
    ...
    for (let field of this.fieldAutoSuggestion) {
      ...
      if (!isInclude&&field?.fieldDataType!=="number") this.filteredFields.push(field);
    }
