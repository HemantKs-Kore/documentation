...
export class BotActionsComponent implements OnInit {
  ...
  filterTable(source, headerOption) {
    ...
    switch (headerOption) {
      case 'isBotName': {
        this.filterSystem.isBotNameFilter = source;
        return;
      }
      case 'isTaskType': {
        this.filterSystem.isTaskTypeFilter = source;
        return;
      }
    }
  }
  ...
  getBots() {
    if (this.streamId) {
      ...
      this.service.invoke('get.bots', quaryparms).subscribe(
        (res) => {
          // console.log(res);
          ...
          if (
            res.tasks &&
            res.tasks.published &&
            res.tasks.published.items &&
            res.tasks.published.items.length
          ) {
  ...
  sortBy(sortingTask: string) {
    ...
    const sortedData = listData.sort((a: any, b: any) => {
      ...
      switch (sortingTask) {
        case 'botName':
          return this.compareValues(a.botName, b.botName, sortOrder);
        case 'type':
          return this.compareValues(a.type, b.type, sortOrder);
        default:
          return;
      }
    });
                ...
                if (dialogsArr.length > 0) {
                  for (let i = 0; i < dialogsArr.length; i++) {
                    if (dialogsArr[i].state == 'published') {
                      element.publishedTasksCount =
                        dialogsArr[i].count + element.publishedTasksCount;
                    }
                  }
                }
                ...
                if (kTasksArr.length > 0) {
                  for (let j = 0; j < kTasksArr.length; j++) {
                    if (kTasksArr[j].state == 'published') {
                      element.publishedFAQsCount =
                        kTasksArr[j].count + element.pusblishedFAQsCount;
                    }
                  }
                }
  ...
  getAssociatedTasks(botID) {
    if (botID != null) {
      ...
      if (this.searchIndexId) {
        ...
        this.loadingContent = false;
        this.service
          .invoke('get.allTasks', queryParams, null, { state: 'published' })
          .subscribe(
            (res) => {
              this.linkedBotTasks = [];
                    ...
                    stream.tasks.published.items.forEach((element) => {
                      if (element.state == 'published') {
                        ...
                        if (this.isBotNameArr.length) {
                          ...
                          if (taskIndex == -1) {
                            this.isBotNameArr.push({
                              botName: element.botName,
                              botIconUrl: element.botIconUrl,
                            });
                          }
                        } else {
                          this.isBotNameArr.push({
  ...
  enableTask(taskID) {
    ...
    if (taskID && this.searchIndexId) {
      ...
      this.service
        .invoke('put.enableTask', queryParams, requestBody, {
          state: 'published',
        })
        .subscribe(
          (res) => {
          ...
          (err) => {
            this.notificationService.notify('Task Enabling Failed', 'error');
          }
        );
    }
  }
        ...
        .subscribe(
          (res) => {
            ...
            if (res.tasks.length > 0) {
              ...
              // console.log("Linked Bot, Tasks", this.linkedBotTasks);
              this.notificationService.notify(
                this.linkedBotTasks.length == requestBody['tasks'].length
                  ? 'All the tasks are enabled'
                  : requestBody['tasks'].length + ' tasks are enabled',
                'success'
              );
              // this.selcectionObj = {
              //   selectAll: false,
  ...
  disableTask(taskID) {
    ...
    if (taskID && this.searchIndexId) {
      ...
      this.service
        .invoke('put.disableTask', queryParams, requestBody, {
          state: 'published',
        })
        .subscribe(
          (res) => {
          ...
          (err) => {
            this.notificationService.notify('Task Disabling Failed', 'error');
          }
        );
    }
  }
        ...
        .subscribe(
          ...
          if (res._id === element._id) {
            ...
            if (res.tasks.length > 0) {
              ...
              this.notificationService.notify(
                this.linkedBotTasks.length == requestBody['tasks'].length
                  ? 'All the tasks are disabled'
                  : requestBody['tasks'].length + ' tasks are disabled',
                'success'
              );
              // this.selcectionObj = {
              //   selectAll: false,
  ...
  syncLinkedBot() {
    if (this.searchIndexId) {
      ...
      this.service
        .invoke('put.syncLinkedBot', queryParams, null, { state: 'published' })
        .subscribe((res) => {
          // console.log(res);
          if (res.tasks.length > 0) {
            this.linkedBotTasks = [];
        ...
        channels: [
          {
            ...
            app: {
              ...
              name: (this.selectedLinkBotConfig.channels || []).length
