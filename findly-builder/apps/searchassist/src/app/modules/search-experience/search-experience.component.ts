...
export class SearchExperienceComponent implements OnInit, OnDestroy {
  ...
  addSearchExperience() {
    ...
    this.service.invoke('put.searchexperience', quaryparms, obj).subscribe(
      (res) => {
        this.store.dispatch(
          setSearchExperienceConfigSuccess({ searchExperienceConfig: res })
        );
  ...
  recentSearches() {
    this.searchObject.searchInteractionsConfig.showSearches = this.searchObject
      .searchInteractionsConfig.showSearchesEnabled
      ? 'recent'
      : 'frequent';
  }
