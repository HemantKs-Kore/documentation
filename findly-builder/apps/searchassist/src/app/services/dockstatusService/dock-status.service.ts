...
export class DockStatusService {
      ...
      const polling = interval(5000)
        ...
        .subscribe(() => {
          this.service.invoke('get.dockstatus', params).subscribe((res) => {
            ...
            if (
              jobObj &&
              (jobObj.status === 'SUCCESS' || jobObj.status === 'success')
            ) {
              polling.unsubscribe();
            ...
            // else if(jobObj && jobObj.status === 'FAILURE') {
              ...
              jobObj &&
              (jobObj.status === 'FAILURE' || jobObj.status === 'FAILED')
            ) {
              polling.unsubscribe();
  ...
  updateProgress(payload, _id: string) {
    ...
    return Observable.create((observer) => {
      this.service.invoke('put.updatedockstatus', _params, payload).subscribe(
        (res) => {
          observer.next(res);
        },
        (err) => {
  ...
  downloadDockFile(fileId, fileName) {
    const params = { fileId };
    this.service.invoke('get.downloaddockfile', params).subscribe(
      (res) => {
