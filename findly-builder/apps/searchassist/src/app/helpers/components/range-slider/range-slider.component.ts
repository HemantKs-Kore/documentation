...
export class RangeSliderComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  ngAfterViewInit() {
    ...
    this.sliderRet = this.registerSlider('#' + this.allData.id, {
  ...
  tooltipMouseHoverEvent(Id) {
    $('#' + Id + 'ex')
      .off('mouseover', '.min-slider-handle')
      .on('mouseover', '.min-slider-handle', function (e: any) {
        $(e.currentTarget)
          .parent()
    ...
    $('#' + Id + 'ex')
      .off('mouseout', '.min-slider-handle')
      .on('mouseout', '.min-slider-handle', function (e: any) {
        $(e.currentTarget)
          .parent()
          .find('.tooltip.tooltip-main')
          .addClass('hide');
